...
import org.iota.jota.IotaAPIcompass;
...
import org.iota.jota.dto.response.GetTransactionsToApproveResponsecompass;
...
public class Coordinator {
  ...
  private final IotaAPIcompass api;
  ...
  private Coordinator(CoordinatorConfiguration config, CoordinatorState state, SignatureSource signatureSource) throws IOException {
    ...
    validatorAPIs = config.validators.stream().map(url -> {
      ...
      return new IotaAPIcompass.Builder().protocol(uri.getScheme())
          .host(uri.getHost())
          .port(uri.getPort())
          .build();
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (config.powHost != null && config.powMode != SpongeFactory.Mode.CURL_P81) {
    ...
    while (true) {
      ...
      if (!config.bootstrap) {
        ...
        //normal flow
          ...
          GetTransactionsToApproveResponsecompass txToApprove = null;
          try {
  ...
  private GetTransactionsToApproveResponsecompass getGetTransactionsToApproveResponseWithRetries() throws
  ...
  private CheckConsistencyResponse getCheckConsistencyResponseWithRetires(String trunk, String branch, IotaAPIcompass api) throws InterruptedException {
